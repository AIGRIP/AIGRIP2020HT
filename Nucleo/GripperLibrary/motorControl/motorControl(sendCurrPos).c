...
void GripperStartupConfiguration (UART_HandleTypeDef *uart_Thumb, UART_HandleTypeDef *uart_Dynamic, UART_HandleTypeDef *uart_Finger1, UART_HandleTypeDef *uart_Finger2, gripperStruct *motors)
{
	...
	MotorInitConfig(uart_Dynamic, &motors->finger2Motor0.id, &motors->finger2Motor0);
	...
	MotorInitConfig(uart_Dynamic, &motors->finger1Motor0.id, &motors->finger1Motor0);
...
void MotorReadMsgQueueFromNano (osMessageQueueId_t nanoNewValuesQueue, motorPositions *positions, motorPair *thumb, motorPair *finger1,
								motorPair *finger2, motorPair *dynamic, gripperStruct *motors, osSemaphoreId_t semaforMotorCommand)
{
	uint16_t tempPos = 0, controlCommand = 0;

	if (osMessageQueueGetCount(nanoNewValuesQueue) > 0)
	{
		osMessageQueueGet(nanoNewValuesQueue, &controlCommand, NULL, 0U);
		switch (controlCommand)
		{
			...
			case 5:
			{
				...
				if (osSemaphoreAcquire( semaforMotorCommand, 200) == osOK)
				{
					...
					{
						osMessageQueueGet(nanoNewValuesQueue, &tempPos, NULL, 0U);
			...
			default:
				while (1) {}
				{
					int numberOfMessageToRemove = osMessageQueueGetCount(nanoNewValuesQueue);
...
void MotorControlThread(UART_HandleTypeDef *uart_Thumb, UART_HandleTypeDef *uart_Dynamic, UART_HandleTypeDef *uart_Finger1,
						UART_HandleTypeDef *uart_Finger2, osMessageQueueId_t nanoNewValuesQueue, osMessageQueueId_t nanoCurrentValuesQueue,
						osMessageQueueId_t opticalSensorQueue, osSemaphoreId_t semaforMotorCommand)
{
	...
	osDelay(5);
	GripperStartupConfiguration(uart_Thumb, uart_Dynamic, uart_Finger1, uart_Finger2, &gripper);



	// make positions linked to gripper struct goalPositions
	gripper.gripperState = GRIPPER_PAUSE;
	motorPositions newPositions = {gripper.gripperState,&gripper.thumbM1.goalPosition, &gripper.thumbM2.goalPosition, &gripper.finger1Motor0.goalPosition, &gripper.finger1Motor1.goalPosition, &gripper.finger1Motor2.goalPosition, &gripper.finger2Motor0.goalPosition, &gripper.finger2Motor1.goalPosition, &gripper.finger2Motor2.goalPosition};
	...
	motorPair dynamic = {uart_Dynamic, 0, 0, &gripper.finger1Motor0, &gripper.finger2Motor0};

//	int switchPos = 0;
	int sendStatus;
	uint64_t currTick1, currTick2, executionTicks;
	uint8_t buf[18] = {0};
...
/*
	osKernelLock();
	MotorSetGoalPosition(dynamic.huart, &dynamic.motor2->id, &tempPos);
	osKernelUnlock();
*/
	//int wander = 0;
	...
	for(;;)
	{
		...
		MotorReadMsgQueueFromNano(nanoNewValuesQueue, &newPositions, &thumb, &finger1, &finger2, &dynamic, &gripper, semaforMotorCommand);

		if (osMessageQueueGetSpace(nanoCurrentValuesQueue) >= NUMBER_OF_MOTORS + 1)
		{

			currentPositions.controlCommand = gripper.gripperState;
			// READ: Gather data of each motor regarding grasp
			MotorRead(thumb.huart, buf, thumb.motor1->id, ADDR_GOAL_POSITION, 2);
			currentPositions.thumbMotor1 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(thumb.huart, buf, thumb.motor2->id, ADDR_GOAL_POSITION, 2);
			currentPositions.thumbMotor2 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(finger1.huart, buf, finger1.motor1->id, ADDR_GOAL_POSITION, 2);
			currentPositions.finger1Motor1 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(finger1.huart, buf, finger1.motor2->id, ADDR_GOAL_POSITION, 2);
			currentPositions.finger1Motor2 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(finger2.huart, buf, finger2.motor1->id, ADDR_GOAL_POSITION, 2);
			currentPositions.finger2Motor1 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(finger2.huart, buf, finger2.motor2->id, ADDR_GOAL_POSITION, 2);
			currentPositions.finger2Motor2 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(dynamic.huart, buf, dynamic.motor1->id, ADDR_GOAL_POSITION, 2);
			currentPositions.dynamicMotor1 = (buf[6] << 8) | buf[5];
			memset(&buf, 0, sizeof(buf));
			MotorRead(dynamic.huart, buf, dynamic.motor2->id, ADDR_GOAL_POSITION, 2);
			currentPositions.dynamicMotor2 = (buf[6] << 8) | buf[5];


			// Send current motor positions to Nano
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.controlCommand, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.thumbMotor1, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.thumbMotor2, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.finger1Motor1, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.finger1Motor2, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.finger2Motor1, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.finger2Motor2, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.dynamicMotor1, 1, 100);
			sendStatus |= osMessageQueuePut(nanoCurrentValuesQueue, &currentPositions.dynamicMotor2, 1, 100);
		...
		switch (gripper.gripperState) {
			...
			case GRIPPER_SLIPNOT:
				IfSlipIncreaseForce(&thumb, &finger1, &finger2, &currentPositions, opticalSensorQueue);
				break;
			// SECTION FOR STOP
			case GRIPPER_STOP:
				// FOR TESTING, STOP COMMAND RETURNS MOTORS TO STARTUP POSITIONS AND CONFIGURATION
				GripperStartupConfiguration(thumb.huart, dynamic.huart, finger1.huart, finger2.huart, &gripper);
/*
				...
				HAL_Delay(5);
				MotorSendStop(dynamic.huart, &dynamic.motor1->id);
				HAL_Delay(5);
				MotorSendStop(dynamic.huart, &dynamic.motor2->id);
				HAL_Delay(5);
...
*/
		...
		{
			osKernelLock();
			MotorSetGoalPosition(thumb.huart, &thumb.motor1->id, newPositions.posThumbMotor1);
			osKernelUnlock();
		...
		{
			osKernelLock();
			MotorSetGoalPosition(thumb.huart, &thumb.motor2->id, newPositions.posThumbMotor2);
			osKernelUnlock();
		...
		{
			osKernelLock();
			MotorSetGoalPosition(finger1.huart, &finger1.motor1->id, newPositions.posFinger1Motor1);
			osKernelUnlock();
		...
		{
			osKernelLock();
			MotorSetGoalPosition(finger1.huart, &finger1.motor2->id, newPositions.posFinger1Motor2);
			osKernelUnlock();
		...
		{
			osKernelLock();
			MotorSetGoalPosition(finger2.huart, &finger2.motor1->id, newPositions.posFinger2Motor1);
			osKernelUnlock();
		...
		{
			osKernelLock();
			MotorSetGoalPosition(finger2.huart, &finger2.motor2->id, newPositions.posFinger2Motor2);
			osKernelUnlock();
			finger2.changedPosMotor2 = 0;
			osDelay(2);
		}

		if (dynamic.changedPosMotor1)
		{
			osKernelLock();
			MotorSetGoalPosition(dynamic.huart, &dynamic.motor1->id, newPositions.posFinger1Motor0);
			osKernelUnlock();
			dynamic.changedPosMotor1 = 0;
			osDelay(2);
		}

		if (dynamic.changedPosMotor2)
		{
			osKernelLock();
			MotorSetGoalPosition(dynamic.huart, &dynamic.motor2->id, newPositions.posFinger2Motor0);
			osKernelUnlock();
			dynamic.changedPosMotor2 = 0;
			osDelay(2);
		}
