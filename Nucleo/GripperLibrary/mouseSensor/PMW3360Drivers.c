...
// Length of firmware for mouse sensor
const unsigned short firmware_length = 4094;

// Firmware for mouse sensor.
...
void SPI_E_ReadReg(uint8_t *REG_ADDR, uint8_t *readByte, SPI_HandleTypeDef *hspiHandle )
{
  ...
  HAL_SPI_Transmit(hspiHandle,(uint8_t *) &READ_REG_ADDR, 1, 100);

  HAL_SPI_Receive(hspiHandle,(uint8_t *) readByte, 1, 100);
...
void SPI_E_WriteReg(uint8_t *REG_ADDR, uint8_t *sendByte, SPI_HandleTypeDef *hspiHandle )
{
  ...
  HAL_SPI_Transmit(hspiHandle,(uint8_t *) sendByte, 1, 100);
...
void startUpMouse(SPI_HandleTypeDef *hspiHandle)
{
  ...
  // Start up register
  sendByte = 0x5A;
  SPI_E_WriteReg(&startUpRegAddr[0], &sendByte ,hspiHandle);

  // Must wait 50 milliseconds.
  osDelay(50);
  // Read and discard data.
  SPI_E_ReadReg(&startUpRegAddr[1], &readByte ,hspiHandle);
  SPI_E_ReadReg(&startUpRegAddr[2], &readByte ,hspiHandle);
  SPI_E_ReadReg(&startUpRegAddr[3], &readByte ,hspiHandle);
  SPI_E_ReadReg(&startUpRegAddr[4], &readByte ,hspiHandle);
  SPI_E_ReadReg(&startUpRegAddr[5], &readByte ,hspiHandle);

  // Upload SROM data to mouse sensor.

  // Write 0 to Rest_En bit of Config2 register to disable Rest mode.
  sendByte = 0x00;
  SPI_E_WriteReg(&startUpRegAddr[6], &sendByte ,hspiHandle);

  // Write 0x1d to SROM_Enable register for initializing.
  sendByte = 0x1D;
  SPI_E_WriteReg(&startUpRegAddr[7], &sendByte ,hspiHandle);
  ...
  sendByte = 0x18;
  SPI_E_WriteReg(&startUpRegAddr[8], &sendByte ,hspiHandle);
  ...
  HAL_SPI_Transmit(hspiHandle, &startUpRegAddr[9], 1, 100);


  for(int i=0;i<firmware_length;i++)
  {
	  HAL_SPI_Transmit(hspiHandle,(uint8_t *) &firmware_data[i], 1, 100);
	  osDelay(1);
  }
  ...
  SPI_E_ReadReg(&startUpRegAddr[10], &readByte ,hspiHandle);

  // Write 0x00 to config2 register for wired mouse design.
  sendByte = 0x00;
  SPI_E_WriteReg(&startUpRegAddr[11], &sendByte ,hspiHandle);

  // Set Config1
  sendByte = 0x77;
  uint8_t conf1Addr = 0x0F;
  SPI_E_WriteReg(&conf1Addr, &sendByte ,hspiHandle);
...
void readMouseSensor(sensValue *sensorPacket, SPI_HandleTypeDef *hspiHandle)
{
	...
	SPI_E_WriteReg(&motionADR[0], &freeze, hspiHandle );
	// Check if any motion has occurred.
	SPI_E_ReadReg(&motionADR[0], &readByte, hspiHandle );
	// Save the motion bit.
	sensorPacket->mouseMotion = 0 < (0x80 & readByte);


	//Read the delta X registers.
	SPI_E_ReadReg(&motionADR[1], &readByte, hspiHandle);
	magnX_L = (uint8_t) readByte;
	SPI_E_ReadReg(&motionADR[2], &readByte, hspiHandle );

	// Merge the lower and higher bit values of the delta X registers.
	sensorPacket->mouse_D_X = magnX_L | (readByte<<8);

	//Read the delta X registers.
	SPI_E_ReadReg(&motionADR[3], &readByte, hspiHandle );
	magnY_L = (uint8_t) readByte;
	SPI_E_ReadReg(&motionADR[4], &readByte, hspiHandle );
